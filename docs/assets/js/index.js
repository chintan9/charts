...
import {getRandomBias, shuffle} from '../../../src/js/utils/helpers';
...
Array.prototype.slice
    ...
    .map(el => {
      el.addEventListener('click', (e) => {
        ...
        Array.prototype.slice.call(btn.parentNode.querySelectorAll('button'))
            .map(el => { el.classList.remove('active'); });
        btn.classList.add('active');
      });
    });

document.querySelector('.export-aggr')
    .addEventListener('click', () => { aggrChart.export(); });
...
let updateData = {
  ...
  yMarkers : [ {label : "Altitude", value : 25, type : 'dashed'} ],
  yRegions : [
    {label : "Range", start : 10, end : 45},
  ],
};
...
chartUpdateButtons.querySelector('[data-update="random"]')
    .addEventListener("click", () => {
      ...
      let data = {
        ...
        datasets : [ {values : getUpdateData(updateDataAllValues)} ],
        yMarkers : [ {label : "Altitude", value : value, type : 'dashed'} ],
        yRegions : [
          {label : "Range", start : start, end : end},
        ],
      };
      updateChart.update(data);
    });

chartUpdateButtons.querySelector('[data-update="add"]')
    .addEventListener("click", () => {
      let index = updateChart.state.datasetLength; // last index to add
      if (index >= updateDataAllIndices.length)
        return;
      updateChart.addDataPoint(updateDataAllLabels[index],
                               [ updateDataAllValues[index] ]);
    });

chartUpdateButtons.querySelector('[data-update="remove"]')
    .addEventListener("click", () => { updateChart.removeDataPoint(); });

document.querySelector('.export-update')
    .addEventListener('click', () => { updateChart.export(); });
...
document.querySelector('.export-trends')
    .addEventListener('click', () => { trendsChart.export(); });
...
Array.prototype.slice
    ...
    .map(el => {
      el.addEventListener('click', (e) => {
        ...
        let colors = [];
        let colors_mode =
            document.querySelector('.heatmap-color-buttons .active')
...
Array.prototype.slice
    ...
    .map(el => {
      el.addEventListener('click', (e) => {
        ...
        let view_mode = document.querySelector('.heatmap-mode-buttons .active')
...
document.querySelector('.export-heatmap')
    .addEventListener('click', () => { heatmapChart.export(); });
